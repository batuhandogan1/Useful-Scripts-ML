...
def main():
    ...
    if input_type == 'image':
        ...
        for image in images:
            ...
            origin_img = np.asarray(Image.open(args.path + image).convert('RGB'))
            bboxes, scores, cls_inds = infer_engine.forward(origin_img)
            vis_res = infer_engine.visualize(origin_img, bboxes, scores, cls_inds, conf=args.conf, save_name=os.path.basename(image), save_result=args.save_result)
